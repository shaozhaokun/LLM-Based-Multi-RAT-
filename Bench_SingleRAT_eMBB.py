...
def _force_embb_single_rat_association(outer, k_urllc, k_embb, sixg_num, wifi_num, sat_num):
    outer = np.asarray(outer, dtype=np.int8).copy()
    uplink_num = int(sixg_num + wifi_num + sat_num)
    sat_start = int(sixg_num + wifi_num)
    sat_down_start = uplink_num

    for e in range(k_embb):
        row = k_urllc + e
        selected = np.flatnonzero(outer[row, :uplink_num] > 0)
        if selected.size == 0:
            raise ValueError(f"eMBB row {e} has no selected RAT before single-RAT conversion.")

        pick = int(selected[0])
        outer[row, :uplink_num] = 0
        outer[row, pick] = 1
        outer[row, sat_down_start : sat_down_start + sat_num] = 0
        if sat_start <= pick < sat_start + sat_num:
            outer[row, sat_down_start + (pick - sat_start)] = 1
...
if __name__=="__main__":
    ...
    random.seed(seed)
    outer_iteration = 1
    pool_dir = os.path.join("Pool_SingleRAT_eMBB", f"Outer{outer_iteration}")
    ...
    outer_solution = _force_embb_single_rat_association(
        outer_solution,
        k_urllc,
        k_embb,
        SixG_BSs_num,
